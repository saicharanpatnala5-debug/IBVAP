    ...
    def generate_synthetic_evidence(
        ...
        if bbox and len(bbox) == 4:
            if bbox[2] > 1.0 or bbox[3] > 1.0:
                norm_bbox = [bbox[0] / 640.0, bbox[1] / 360.0, bbox[2] / 640.0, bbox[3] / 360.0]
            else:
                norm_bbox = list(bbox)
        return self.generate_evidence_frame(
            camera_id=camera_id,
            event_title=f"{title} (SCORE: {risk_score})",
            bbox=norm_bbox,
            plate_text=extra_text
        )

evidence_capture = EvidenceCaptureService()
evidence_capture_service = evidence_capture

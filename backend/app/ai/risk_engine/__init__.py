...
class RiskScoringEngine:
    def __init__(self):
        self.weights = {
            "zone_intrusion": settings.WEIGHT_ZONE_INTRUSION,
            "night_context": settings.WEIGHT_NIGHT_CONTEXT,
            "loitering": settings.WEIGHT_LOITERING,
            "inward_direction": settings.WEIGHT_INWARD_DIRECTION,
            "unknown_vehicle": settings.WEIGHT_UNKNOWN_VEHICLE,
            "multiple_objects": settings.WEIGHT_MULTIPLE_OBJECTS
        }
    ...
    def evaluate_risk(
        ...
        if is_zone_intrusion:
            ...
            total_score += pts
            contributing_factors.append({"factor": "Restricted Zone Intrusion", "points": pts, "confidence": 0.95})
        ...
        if is_night_time:
            ...
            total_score += pts
            contributing_factors.append({"factor": "Night-Time Surveillance Window", "points": pts, "confidence": 0.98})
            factor_explanations.append("Movement detected during low-visibility dark operational hours")

        if is_loitering:
            pts = self.weights["loitering"]
            total_score += pts
            contributing_factors.append({"factor": "Prolonged Loitering", "points": pts, "confidence": 0.91})
        ...
        if is_inward_movement:
            ...
            total_score += pts
            contributing_factors.append({"factor": "Inward Vector Toward Border Perimeter", "points": pts, "confidence": 0.88})
        ...
        if is_unknown_vehicle:
            ...
            total_score += pts
            contributing_factors.append({"factor": "Unregistered / Watchlist Vehicle Proximity", "points": pts, "confidence": 0.89})
        ...
        if has_multiple_objects:
            ...
            total_score += pts
            contributing_factors.append({"factor": "Multiple Correlated Entities", "points": pts, "confidence": 0.92})
        ...
        if additional_factors:
            for af in additional_factors:
                ...
                total_score += pts
                contributing_factors.append({"factor": af.get("name", "Supplemental Signal"), "points": pts, "confidence": af.get("confidence", 0.85)})

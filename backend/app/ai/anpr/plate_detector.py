...
class LicensePlateDetector:
    ...
    def detect_plate(self, vehicle_crop: np.ndarray) -> Optional[PlateDetectionResult]:
        ...
        if vehicle_crop is None or vehicle_crop.size == 0:
            ...
            return PlateDetectionResult(plate_crop=sim_crop, bbox=[20, 60, 160, 100], confidence=0.50)

        vh, vw = vehicle_crop.shape[:2]
        if vh < 20 or vw < 20:
            return PlateDetectionResult(plate_crop=vehicle_crop, bbox=[0, 0, vw, vh], confidence=0.45)
        ...
        return PlateDetectionResult(plate_crop=fallback_crop, bbox=[bx1, by1, bx2, by2], confidence=0.85)
